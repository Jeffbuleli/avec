-- McBuleli Congo Mining Simulator — 15 tables
...
CREATE INDEX IF NOT EXISTS game_world_events_active_idx ON game_world_events (active, ends_at);
...
CREATE UNIQUE INDEX IF NOT EXISTS game_mineral_stocks_player_mineral ON game_mineral_stocks (player_id, mineral_key);
...
CREATE INDEX IF NOT EXISTS game_vehicles_player_idx ON game_vehicles (player_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS game_inventory_player_item ON game_inventory (player_id, item_key);
...
-- Seed mineral prices (idempotent)
INSERT INTO game_economy_prices (mineral_key, label, base_price_mcb, current_price_mcb)
VALUES
  ('cobalt', 'Cobalt', 12, 12),
  ('copper', 'Copper', 6, 6),
  ('gold', 'Gold', 45, 45),
  ('coltan', 'Coltan', 18, 18),
  ('lithium', 'Lithium', 14, 14),
  ('diamonds', 'Diamonds', 80, 80)
